...
class Config:
    def __init__(self, train, test, lr0, lr, bs, mask_w, seed, gpu):
        self.train = train
        self.test = test
        self.lr0 = lr0
        self.lr = lr
        ...
        epoch_steps = 124999//self.bs+1 #124999
        self.warmup_steps = 2*epoch_steps
        self.training_steps = 5*epoch_steps #30
        self.warmup_steps0 = 5*epoch_steps #5
        self.training_steps0 = 75*epoch_steps #75
        self.hidden_dim = 768
    
    def learning_rate(self, stage):
        lr = self.lr0 if stage == 1 else self.lr#对第一阶段训练的参数学习率做特殊调整
        return lr
    
    def batch_size(self, mode):
        bs = self.bs
        if not mode:
            bs *= 4 #验证集batch_size是训练集的4倍
        return bs
    ...
    def prediction_path(self):
        ...
        return '{}/result/{}.csv'.format(self.path, store_name)
