...
def data_process():
    ...
    train_dataset = []
    ylabel = [] # 增加一个List ylabel 存储训练集的类别
...
class DataSet(data.Dataset):
    def __init__(self, args, data, mode='train'):
        self.data = data
        self.mode = mode
        self.w2v_model = gensim.models.word2vec.Word2Vec.load("/home/zyf/Summer game2021/Datafountain/competition-baseline-main/全球人工智能技术创新大赛/赛道一/embedding/w2v.model")
...
class HAN(nn.Module):
    def __init__(self,args, vocab_size, embedding_dim, embeddings=None):
        ...
        if embeddings:
            w2v_model = gensim.models.word2vec.Word2Vec.load("/home/zyf/Summer game2021/Datafountain/competition-baseline-main/全球人工智能技术创新大赛/赛道一/embedding/w2v.model").wv
            fasttext_model = gensim.models.word2vec.Word2Vec.load("/home/zyf/Summer game2021/Datafountain/competition-baseline-main/全球人工智能技术创新大赛/赛道一/embedding/fasttext.model").wv
...
def simple_accuracy(preds, labels):
    return (np.array(preds)==np.array(labels)).astype(int).mean()
...
def validation_funtion(model, valid_dataloader, valid_torchdata, mode='valid'):
    ...
    if mode == 'valid':
        ...
        # auc = roc_auc_score(labels_list, pred_list)
        # f1 = f1_score(labels, pred, average='macro')
        acc, f1 = acc_and_f1(y_preds, y_trues)

        return acc, f1, np.mean(val_loss)
    else:
...
#     opt = SWA(optimizer, swa_start=100, swa_freq=5, swa_lr=1e-4)
    ...
    for epoch in range(epochs):
        ...
        # pgd = PGD(model, emb_name="embed.weight", epsilon=1.0)
...
#         opt.swap_swa_sgd()
#         ema.apply_shadow()
        acc, f1, val_loss = validation_funtion(model, valid_dataloader, valid_torchdata, 'valid')
#         ema.restore()
        print('train_loss: {:.5f}, val_loss: {:.5f}, acc: {:.5f}, f1: {:.5f}\n'.format(train_loss[-1], val_loss, acc, f1))
        if early_stop:
...
#                 ema.apply_shadow()
                torch.save(model.state_dict(), 
                    '/home/zyf/Summer game2021/Datafountain/competition-baseline-main/全球人工智能技术创新大赛/赛道一/learned_model/{}_model_{}.bin'.format(args.NAME, model_num))
#                 ema.restore()
                model_num += 1
            else:
                no_improve += 1
...
def run(args, train_dataset, w2v_model, fasttext_model, ylabel):
    ...
    model = HAN(args, w2v_model.wv.vectors.shape[0]+1, w2v_model.wv.vectors.shape[1]+fasttext_model.wv.vectors.shape[1], embeddings=True)
    model.to(DEVICE)
    ...
    for i in range(args.FOLD):
        print('- 第{}折中，best mlogloss: {}   best acc: {}  best f1:{}  best loss: {}'.format(i+
            1, best_mlogloss[i], best_acc[i], best_f1[i], best_loss[i]))
    return model_n
...
def get_submit(args, test_data, test_dataset, id2label, model_num):
    ...
    for i in range(1, model_num):
        model.load_state_dict(torch.load(
            '/home/zyf/Summer game2021/Datafountain/competition-baseline-main/全球人工智能技术创新大赛/赛道一/learned_model/{}_model_{}.bin'.format(args.NAME, i)))
    ...
    submit_file = '/home/zyf/Summer game2021/Datafountain/submits/submit_HAN_PGD.csv'
...
def arg_setting():
    ...
    parser.add_argument('--NAME', default='HANNet', type=str, help="")
