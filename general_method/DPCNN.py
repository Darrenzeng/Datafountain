...
class DataSet(data.Dataset):
    def __init__(self, args, data, mode='train'):
        self.data = data
        self.mode = mode
        self.w2v_model = gensim.models.word2vec.Word2Vec.load(
            "./embedding/w2v.model")
    ...
    def get_data(self, data, mode):
        ...
        for data_li in tqdm(data):
            ...
            text = [self.w2v_model.wv.key_to_index[s] +
                    1 if s in self.w2v_model.wv else 0 for s in text]
...
def run(args, train_dataset, w2v_model, fasttext_model, ylabel):
    ...
    best_loss = []
    model_num = 1
    model = DPCNN(args, w2v_model.wv.vectors.shape[0]+1, w2v_model.wv.vectors.shape[1] +
    ...
    for i in range(args.FOLD):
        print('- 第{}折中，best valloss: {}   best f1-socre: {}   best trainloss: {}'.format(
...
def arg_setting():
    ...
    parser.add_argument('--FOLD', default=20, type=int, help="k fold")
