...
import sys
sys.path.append('/home/zyf/Summer_game2021/Datafountain/liu_bert-pytorch/bert_finetuning_1/')
...
class data_generator:
    def __init__(self, data, config, shuffle=False):
        ...
        vocab = 'vocab.txt' if os.path.exists(
            config.model_path + 'vocab.txt') else 'spiece.model'
        self.tokenizer = TOKENIZERS[config.model].from_pretrained(
            config.model_path + vocab)
    ...
    def __iter__(self):
        ...
        for index, data_li in enumerate(self.data):
            ...
            if len(input_ids) == self.batch_size or index == len(self.data)-1:
                input_ids = paddingList(
                    input_ids, 0, returnTensor=True)  # 动态padding
