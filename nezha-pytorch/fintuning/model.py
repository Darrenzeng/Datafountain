...
class NEZHA(nn.Module):
    ...
    def forward(self, input_ids, input_masks, segment_ids):
        ...
        sequence_output, pooler_output = self.bert_model(input_ids=input_ids, token_type_ids=segment_ids,  # [32, 100, 768]    [32, 768]
                                                                        attention_mask=input_masks)
        seq_avg = torch.mean(sequence_output, dim=1)
        concat_out = torch.cat((seq_avg, pooler_output), dim=1) #  [32, 1536]
